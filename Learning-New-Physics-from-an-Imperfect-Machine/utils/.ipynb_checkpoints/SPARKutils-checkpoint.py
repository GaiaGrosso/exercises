...
class KernelMethod(nn.Module):
    ...
    def mask_inactive_kernels(self,K_x):
        ...
        total_kernel_activation = torch.abs(total_kernel_activation- torch.mean(total_kernel_activation))/torch.std(total_kernel_activation)
        mask_non_active_kernels = 1*(total_kernel_activation<3)
...
class KernelMethod_SoftMax_2(nn.Module):
    ...
    def call(self, x):
        ...
        total_kernel_activation = torch.sum(K_x, dim=0).unsqueeze(1) #[M, 1]
        total_kernel_activation = torch.abs(total_kernel_activation- torch.mean(total_kernel_activation))/torch.std(total_kernel_activation)
        mask_non_active_kernels = 1*(total_kernel_activation<2)
        self.coeffs.data = self.coeffs.data*mask_non_active_kernels
        W_x = self.coeffs  # [M, 1 ] 
